...
┌──────────────────────────────────────────────┐
│ Place on the volar (palm-side) surfaces     │
│ of the index and middle fingers of          │
│ the non-dominant hand.                      │
└──────────────────────────────────────────────┘
Tips:
• Use pre-gelled isotonic EDA electrodes or apply isotonic gel
  to the contact surface as instructed in the lab manual.
• Attach to the middle phalanx (middle segment) of each finger.
• Snug but not tight — do not restrict blood flow or cause pain.
• Do not apply hand lotion before the session.
• Allow isotonic gel to fully wet the skin before starting.

Webcam
┌──────────────────────────────────────────────┐
│ Position so the participant’s face is       │
│ centered, with even front lighting.         │
│ Avoid strong backlighting or glare.         │
└──────────────────────────────────────────────┘
"""
    ...
    def _step_hardware(self):
        items = [
            ...
            ("Electrodes",
             ...
             "EDA leads with pre-gelled isotonic electrodes attached."),
    ...
    def _step_launch(self):
        ...
        step_defs = [
            ...
            ("electrodes", "Electrode leads connected",
             ...
             "and EDA leads with pre-gelled isotonic electrodes."),
            ...
            ("pavlovia", "Pavlovia experiment opened in browser",
             "Open the correct task URL for this participant's group.",
             "🌐 Open Pavlovia"),
        ...
        def _show_urls(url_lines, grp_val):
            win = tk.Toplevel(self._content)
            win.title("Pavlovia Task URLs")
            win.configure(bg=BG)
            win.geometry("560x320")
            tk.Label(win, text=f"Pavlovia URLs — Group {grp_val}",
                     bg=BG, fg=TEXT, font=(FONT_FAMILY, 14, "bold")).pack(pady=(14, 4))
            for line in url_lines:
                task_name, url = line.split(":\n ")
                row = tk.Frame(win, bg=BG2)
                row.pack(fill="x", padx=16, pady=4)
                tk.Label(row, text=task_name, bg=BG2, fg=TEXT2,
                         font=(FONT_FAMILY, 9, "bold"),
                         width=28, anchor="w").pack(side="left")
                url_lbl = tk.Label(row, text=url, bg=BG2, fg=ACCENT2,
                                   font=("Courier", 9), cursor="hand2")
                url_lbl.pack(side="left", padx=4)
                url_lbl.bind("<Button-1>", lambda e, u=url: _wb.open(u))
            styled_button(win, "Close", win.destroy, style="ghost", width=10).pack(pady=12)
            ...
            def _cmd():
                if key == "bridge":
                    ...
                    for i, task in enumerate(TASK_ORDER):
                        url = ls_url(grp, i)
                        url_lines.append(f"{TASK_LABELS[task]}:\n {url}")
                    default_url = ls_url(grp, 0)
                    try:
                        _wb.open(default_url)
        ...
        for item in step_defs:
            ...
            if has_btn:
                btn_label = item[3]
                btn = styled_button(row, btn_label,
        ...
        # Pavlovia URL quick reference
        url_ref = card_frame(self._content)
        url_ref.pack(fill="x", pady=(6, 2))
        tk.Label(url_ref, text="PAVLOVIA URLS FOR THIS SESSION",
                 bg=BG2, fg=TEXT, font=(FONT_FAMILY, 9, "bold")).pack(
        ...
        for i, task in enumerate(TASK_ORDER):
            ...
            tk.Label(row, text=f"{task_label}:", bg=BG2, fg=TEXT2,
                     font=(FONT_FAMILY, 9, "bold"),
                     width=30, anchor="w").pack(side="left")
            lnk = tk.Label(row, text=url, bg=BG2, fg=ACCENT2,
                           font=("Courier", 9), cursor="hand2")
            lnk.pack(side="left", padx=4)
            lnk.bind("<Button-1>", lambda e, u=url: _wb2.open(u))
        tk.Label(url_ref, text="(click any URL to open in browser)",
                 bg=BG2, fg=TEXT2, font=(FONT_FAMILY, 8)).pack(
