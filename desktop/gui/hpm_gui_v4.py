...
class SignalCanvas(tk.Canvas):
    ...
    def push(self, value):
        ...
        if not self._pending_redraw:
            ...
            self._redraw_id = self.after(33, self._redraw)   # throttle to ~30 fps
    ...
    def reset(self):
        ...
        # Cancel any pending redraw scheduled by the old poll loop
        ...
        self.delete("all")
        self._pending_redraw = False
        # Immediately draw the "Waiting…" placeholder so the canvas looks clean
            ...
            try:
                if self.ser.in_waiting:
                    ...
                    if self._TTL_RE.match(line):
                        continue
                    parts = line.split(",")
                    # Arduino serial field order (matches bridge CSV header):
                    #   0: ArduinoSystemTime
                    #   1: ArduinoTime_ms   (timestamp — do NOT use as signal)
                    #   2: RawGSR           (ADC counts)
                    #   3: GSR_uS           (calibrated µS)
                    #   4: RawECG           (ADC counts)
                    #   5: ECG_mV           (processed mV)
                    #   6: eventMarker
                    if len(parts) >= 5:
                        try:
                            gsr_us  = float(parts[2].strip())   # GSR_uS  ← col 2 in v3.3 (ms,rawGSR,GSR_uS,rawECG,ECG_mV)
                            ecg_raw = float(parts[3].strip())   # RawECG ← col 3 in v3.3
...
class SessionMonitor(tk.Frame):
    ...
    def _poll_reader(self):
        ...
        # ── GSR: convert ADS1115 raw counts → µS, then EMA low-pass ────────
        for v in gsr_vals:
            try:
            ...
            # Only update EMA if value is physiologically plausible
            # (corrupt serial lines produce near-zero ADC → skip them)
            # Accept any non-zero ADC-derived µS reading
            if v_uS > 0.0:
                if self._gsr_ema == 0.0:
                    self._gsr_ema = v_uS  # warm-start
                else:
...
class SignalQualityPage(tk.Frame):
    def __init__(self, parent, on_pass=None):
        ...
        self._ecg_ok  = False
        self._gsr_ok  = False
        # Signal/filter state — always initialised here so _connect() resets are reliable
        self._ecg_buf  = []
        self._gsr_ema  = 0.0
        self._ecg_sos  = None
        self._ecg_zi   = None
        self._poll_id  = None   # after() handle — cancelled on every reconnect
        self._gen      = 0      # generation counter — incremented on every _connect()
        self._build()
    ...
    def _connect(self):
        ...
        # ── Full state reset before every new connection ──────────────────
        if self.reader:
            self.reader.stop()
            self.reader = None

        # Cancel any running poll loop so only ONE loop runs at a time
        if self._poll_id is not None:
            self.after_cancel(self._poll_id)
            self._poll_id = None

        # Clear all signal/filter state so second (and later) runs are clean
        self._ecg_buf  = []
        self._gsr_ema  = 0.0
        self._ecg_sos  = None   # force rebuild of SOS coefficients
        self._ecg_zi   = None   # force warm-start from first sample
        self.ecg_cv.reset()
        self.gsr_cv.reset()
        ...
        # Bump generation — any _poll() still in flight with old gen will self-cancel
        self._gen += 1
        my_gen = self._gen
        ...
        # Use after() — never call _poll() directly — so it runs after this
        # method fully returns and no stale after() callbacks can interleave.
        self._poll_id = self.after(80, lambda: self._poll(my_gen))

    def _poll(self, gen=None):
        self._poll_id = None   # clear handle — will be reset by after() at end
        # Stale generation means _connect() has already started a new session
        ...
        if ecg and NUMPY_AVAILABLE:
            ...
            if getattr(self, '_ecg_sos', None) is not None:
                ...
                # Warm-start zi from first sample whenever _ecg_zi is None
                # (set to None on every _connect() to guarantee a clean start)
                if self._ecg_zi is None and len(chunk) > 0:
                    self._ecg_zi = sosfilt_zi(self._ecg_sos) * chunk[0]
                elif self._ecg_zi is None:
                    self._ecg_zi = sosfilt_zi(self._ecg_sos) * 0.0
                filtered, self._ecg_zi = sosfilt(
                    self._ecg_sos, chunk, zi=self._ecg_zi)
                for v in filtered:
        ...
        for v in gsr:
            if 0.05 <= v <= 200.0:
                ...
                if _ema is None or _ema == 0.0:
                    ...
                    self._gsr_ema = _ema * 0.98 + v * 0.02
